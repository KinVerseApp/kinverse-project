-- ---------------------------------------------------------------------------
-- Kinship queries for PGAdmin / plain SQL execution.
-- IMPORTANT: PGAdmin does NOT support psql meta-commands such as \echo, \if,
-- or the :variable syntax used by psql.
-- Replace the UUID below with the person_id you want to inspect.
-- ---------------------------------------------------------------------------

WITH params AS (
  SELECT '11111111-1111-1111-1111-111111111111'::uuid AS person_id
),
ancestors AS (
  SELECT person_a_id AS ancestor_id, 1 AS depth
  FROM relationship_edge
  WHERE edge_type = 'parent_child' AND status = 'confirmed'
    AND person_b_id = (SELECT person_id FROM params)
  UNION ALL
...
FROM ancestors a
JOIN person p ON p.id = a.ancestor_id
ORDER BY a.depth;
...
descendants AS (
  ...
  FROM relationship_edge
  WHERE edge_type = 'parent_child' AND status = 'confirmed'
    AND person_a_id = (SELECT person_id FROM params)
  UNION ALL
...
FROM descendants d
JOIN person p ON p.id = d.descendant_id
ORDER BY d.depth;
...
WHERE mine.person_b_id = (SELECT person_id FROM params)
...
JOIN person p ON p.id = CASE
  WHEN re.person_a_id = (SELECT person_id FROM params) THEN re.person_b_id
  ELSE re.person_a_id
END
WHERE re.edge_type = 'partner' AND re.status = 'confirmed'
  AND (SELECT person_id FROM params) IN (re.person_a_id, re.person_b_id)
...
aunts_uncles AS (
  ...
  WHERE re.edge_type = 'parent_child' AND re.status = 'confirmed'
    ...
    AND re.person_b_id <> (SELECT person_id FROM params)
)
SELECT p.first_name, p.last_name
FROM aunts_uncles au
...
my_partner AS (
  SELECT CASE WHEN person_a_id = (SELECT person_id FROM params) THEN person_b_id ELSE person_a_id END AS partner_id
  FROM relationship_edge
  WHERE edge_type = 'partner' AND status = 'confirmed' AND end_date IS NULL
    AND (SELECT person_id FROM params) IN (person_a_id, person_b_id)
)
